...
def convert_checkpoint(checkpoint_path, config_path, output_dir, push_to_hub=None):
    ...
    if push_to_hub:
        ...
        model.push_to_hub(
            push_to_hub, commit_message="Convert ISCA checkpoint to HuggingFace format"
        )
        tokenizer.push_to_hub(
            push_to_hub, commit_message="Add tokenizer for ISCA model"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ISCA checkpoint to HuggingFace format"
    )
    parser.add_argument(
        "--checkpoint", type=str, required=True, help="Path to ISCA checkpoint"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/default.yaml",
        help="Path to ISCA config file",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Push model to HuggingFace Hub with this repo ID",
    )
