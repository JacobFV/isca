...
from .visualizations.plots import visualize_centroids, visualize_metrics
...
def main(args):
    ...
    cfg_m, cfg_t, cfg_l = cfg_all.get("model", {}), cfg_all.get("train", {}), cfg_all.get("loss", {})
    
    # Create eval dataset
    ds = TextDataset(args.eval_data or cfg_t.get("dataset"), cfg_m.get("backbone"), cfg_t.get("max_seq"))
    ...
    if args.eval_type == "basic":
        ...
        if args.visualize:
            visualize_metrics(metrics, save_path=save_path / "metrics.png" if save_path else None)
    
    elif args.eval_type == "centroids":
        # Run centroid visualization
        visualize_centroids(model, save_path=save_path / "centroids.png" if save_path else None)
    
    elif args.eval_type == "graph":
        # Dynamically import and run graph structure analysis
        from .analysis.graph import analyze_graph_structure
        ...
        from .analysis.graph import analyze_graph_memory
        ...
        # Run MMLU benchmark
        from .benchmarks import evaluate_mmlu
        evaluate_mmlu()
    
    elif args.eval_type == "mmlu_pro":
        # Run MMLU-Pro benchmark
        from .benchmarks import evaluate_mmlu_pro
        evaluate_mmlu_pro()
    
    elif args.eval_type == "real_bench":
        # Run REAL Bench benchmark
        from .benchmarks import evaluate_real_bench_cli
        ...
        try:
            ...
            if hasattr(custom_module, "main"):
                ...
                print(f"Error: Custom module {args.custom_module} does not have a 'main' function.")
        except ImportError as e:
            print(f"Error importing custom module: {e}")
    
    else:
        print(f"Unknown evaluation type: {args.eval_type}")
        print("Available evaluation types: basic, centroids, graph, memory, symbolic, operators, roles, mmlu, mmlu_pro, real_bench, custom")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_type", type=str, default="basic", 
                        help="Type of evaluation to run (basic, centroids, graph, memory, symbolic, operators, roles, mmlu, mmlu_pro, real_bench, custom)")
    parser.add_argument("--custom_module", type=str, default=None, help="Path to custom evaluation module (for eval_type=custom)")
